...
class ESP32ActionServer(Node, NodeFather, ActionParentServer):

    def __init__(self):
        Node.__init__(self,'esp32_inf_server')
        NodeFather.__init__(self)
        #Se da como parámetro el self, el action_type y el action_name
        ActionParentServer.__init__(self, BinCharge, 'esp32_inf')
    ...
    def execute_callback(self,goal_handle):
        try:
            ...
            port = '/dev/esp32' # TODO: Aquí hay que hacer una función que administre los puertos para cuando haya varias esp32 conectados
            ...
            file_path = goal_handle.request.path_bin #Ruta de archivo .bin
            ...
            self.send_feedback('Executing upload to ESP32')#Feedback

            #Ejecuta el comando desde el método de clase
            ActionParentServer.execute_command(self, 'esp32', port, file_path)
...
def main(args=None):
    ...
    esp32_inf_server = ESP32ActionServer()
    rclpy.spin(esp32_inf_server)
